...
air_monitoring_model = joblib.load("air_ebm_model.pkl")
water_monitoring_model = joblib.load("water_ebm_model.pkl")
...
def predict(data: InputData):
    ...
    # Make prediction
    prediction = int(air_monitoring_model.predict(input_array)[0])
    probabilities = air_monitoring_model.predict_proba(input_array)[0].tolist()

    # Get local explanation
    explanation = air_monitoring_model.explain_local(input_array, [0])
...
def predict(data: InputData):
    ...
    # Make prediction
    prediction = int(water_monitoring_model.predict(input_array)[0])
    probabilities = water_monitoring_model.predict_proba(input_array)[0].tolist()

    # Get local explanation
    explanation = water_monitoring_model.explain_local(input_array, [0])
    raw_scores = explanation.data(0)['scores']
    feature_names = explanation.data(0)['names']
